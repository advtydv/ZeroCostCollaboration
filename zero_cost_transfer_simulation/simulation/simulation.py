...
class SimulationManager:
    ...
    def _expire_temporary_state(self):
        """Expire stale access grants and overdue tasks at the start of each round."""
        for agent_id, agent in self.agents.items():
            self._expire_access_grants_for_agent(agent_id, agent)
            self._expire_overdue_tasks_for_agent(agent_id, agent)

    def _expire_access_grants_for_agent(self, agent_id: str, agent):
        """Expire temporary access grants whose validity window has elapsed."""
        expired_info = [
            info_name
            for info_name, details in list(agent.access_grants.items())
            if details.get('expires_at_round') is not None
            and self.current_round > details['expires_at_round']
        ]

        for info_name in expired_info:
            details = agent.access_grants.pop(info_name)
            from_agent = details['from_agent']

            sender = self.agents.get(from_agent)
            if sender is not None:
                sender.sent_information[agent_id].discard(info_name)
                if not sender.sent_information[agent_id]:
                    sender.sent_information.pop(agent_id, None)
                if sender.mode == "perfect":
                    sender.perfect_responded.discard((agent_id, info_name))

            if agent.mode == "perfect":
                agent.perfect_requested.discard((from_agent, info_name))

            self.communication.send_message(
                'system',
                agent_id,
                f"ACCESS EXPIRED: Your authorization to use {info_name} from {from_agent} has expired."
            )
            self.sim_logger.log_event('access_grant_expired', {
    ...
    def _expire_overdue_tasks_for_agent(self, agent_id: str, agent):
        """Expire deliverables that have exceeded their service window and replace them."""
        overdue_tasks = [
            task for task in list(agent.tasks)
            if task.get('deadline_round') is not None
            and self.current_round > task['deadline_round']
        ]

        for task in overdue_tasks:
            agent.tasks = [t for t in agent.tasks if t['id'] != task['id']]
            self.communication.send_message(
                'system',
                agent_id,
                f"SERVICE WINDOW EXPIRED: Deliverable {task['id']} was not completed by round "
                f"{task['deadline_round']} and has been replaced."
            )
            self.sim_logger.log_event('task_expired', {
                'round': self.current_round,
                'agent_id': agent_id,
                'task_id': task['id'],
                'deadline_round': task.get('deadline_round'),
            })

            replacement_task = self.task_manager.create_task(agent_id)
            agent.assign_task(replacement_task)
    ...
    def _process_action(self, agent_id: str, action: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if action_type == 'send_message':
            ...
            if custom_values is None:
                ...
                self.logger.warning(f"Agent {agent_id} granted access without values field - using original values")
    ...
    def _process_information_transfer(self, from_agent: str, to_agent: str, information: List[str], 
        ...
        if missing_info:
            self.logger.warning(f"Agent {from_agent} tried to grant access to information they don't have: {missing_info}")
            return {'success': False, 'error': f'You do not have: {", ".join(missing_info)}'}
        
        # Grant access without duplicating the information into the receiver's inventory.
        transferred = []
        transferred_with_values = {}  # Track what values were actually sent
        
        for info_name in information:
            # Check if receiver already has a piece with this name or already has granted access
            receiver_has_name = (
                any(piece.name == info_name for piece in receiver.information)
                or info_name in receiver.access_grants
            )
            ...
            if not receiver_has_name:
                ...
                if sender_pieces:
                    ...
                    receiver.access_grants[info_name] = {
                        'from_agent': from_agent,
                        'value': transfer_value,
                        'quality': original_piece.quality,
                        'category': original_piece.category,
                        'granted_round': self.current_round,
                        'expires_at_round': self.current_round + int(
                            self.config.get('tasks', {}).get('access_grant_validity_rounds', 2)
                        ),
                    }
                    ...
                    self.logger.info(
                        f"Granted access to '{info_name}' (quality: {original_piece.quality}, value: {transfer_value}) "
                        f"from {from_agent} to {to_agent}"
                    )
            else:
                self.logger.info(f"Receiver {to_agent} already has access to '{info_name}', skipping grant")
        ...
        if transferred and self.config['simulation'].get('show_full_revenue', True):
            ...
            notification = f"Access granted from {from_agent}: {', '.join(transferred)}"
    ...
    def _process_task_submission(self, agent_id: str, answer: Any) -> Dict[str, Any]:
        ...
        agent_info_names.update(agent.access_grants.keys())
        ...
        if self.task_manager.check_answer(current_task, answer):
            ...
            for info_name in current_task['required_info']:
                local_piece = next((p for p in agent.information if p.name == info_name), None)
                if local_piece:
                    used_piece_records.append({
                        'name': local_piece.name,
                        'quality': local_piece.quality,
                        'value': local_piece.value,
                        'received_from': 'original',
                    })
                elif info_name in agent.access_grants:
                    grant = agent.access_grants[info_name]
                    used_piece_records.append({
                        'name': info_name,
                        'quality': grant['quality'],
                        'value': grant['value'],
                        'received_from': grant['from_agent'],
                    })

            quality_avg = sum(piece['quality'] for piece in used_piece_records) / len(used_piece_records) if used_piece_records else 100
            value_avg = sum(piece['value'] for piece in used_piece_records) / len(used_piece_records) if used_piece_records else 50
            ...
            for info_name in current_task['required_info']:
                ...
                if original_pieces:
                    ...
                    grant = agent.access_grants.get(info_name)
                    if grant:
                        true_values[info_name] = grant['value']
            
            for piece in used_piece_records:
                ...
                value_details[piece['name']] = {
                    ...
                    'received_from': piece['received_from'],
                ...
                if agent_value != true_value:
                    incorrect_values.append({
                        ...
                        'received_from': piece['received_from'] if piece['received_from'] != 'original' else 'unknown'
                    })
                            ...
                            if target_has_info and target_piece:
                                ...
                                if (requester_id, requested_info_name) not in target.perfect_responded:
                                    ...
                                    if transfer_result['success']:
                                        ...
                                        self.logger.info(
                                            f"INSTANT ACCESS GRANT (perfect mode): '{requested_info_name}' "
