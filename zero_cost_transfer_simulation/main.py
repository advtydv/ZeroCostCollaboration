...
def main():
    ...
    parser.add_argument('--log-level', type=str, default='INFO',
                        ...
                        help='Logging level')
    parser.add_argument('--output-dir', type=str, default=str(base_dir / 'logs'),
    ...
    parser.add_argument('--model', type=str, default=None,
                        help='Override the model in the config')
    ...
    if args.model:
        ...
        logger.info(f"Overriding model via CLI: {args.model}")
    ...
    if seed is not None:
        ...
        try:
            ...
            logger.debug("NumPy not installed; skipping NumPy RNG seeding")
    
    # Load variant if specified
    variant_overlay = {}
    if args.variant:
        from variants.variant_loader import VariantLoader
        variant_overlay = VariantLoader.load_variant(args.variant)
        if variant_overlay:
            logger.info(f"Loaded variant: {args.variant}")
    ...
    print(f"\nTotal Revenue (all agents): ${total_revenue:,}")
