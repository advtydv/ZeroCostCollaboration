...
class Agent:
    ...
    def __init__(self, agent_id: str, config: dict, initial_info: List,
        ...
        self.prompt_ablation = getattr(simulation_manager, "prompt_ablation", None)
...
"""
        ...
        if self.prompt_ablation:
            return self.prompt_ablation.transform_prompt(prompt)
        return prompt
    ...
    def _format_current_task(self) -> str:
        ...
        for i, task in enumerate(self.tasks, 1):
            ...
            if self.prompt_ablation:
                description = self.prompt_ablation.task_description(task)
            task_lines.append(
...
Your goal is maximizing the system's overall revenue. Cooperate with the other agents to achieve this goal."""
    ...
    def _generate_cooperation_scores_example(self, revenue_board: Dict[str, int]) -> str:
        ...
        import hashlib
        import random
        seed_material = f"{self.agent_id}:{len(all_agents)}".encode("utf-8")
        deterministic_seed = int(hashlib.sha256(seed_material).hexdigest()[:16], 16)
...
Return exactly one valid JSON object and nothing else; no explanation, no self-correction, no second attempt, and no markdown code fences."""
        ...
        if self.prompt_ablation:
            report_prompt = self.prompt_ablation.transform_prompt(report_prompt)
    ...
    def _parse_actions(self, response: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if 'actions' not in data:
                ...
                if 'action' in data:
                    translated = self.prompt_ablation.translate_action_to_internal(data) if self.prompt_ablation else data
                    validated = self._validate_single_action(translated)
            ...
            for i, action in enumerate(data['actions']):
                translated = self.prompt_ablation.translate_action_to_internal(action) if self.prompt_ablation else action
                validated = self._validate_single_action(translated)
                if validated:
